...
import { PlusCircle, Loader2, Wand2, Clipboard, MoreHorizontal, Eye, PauseCircle, PlayCircle, Trash2 } from "lucide-react";
import { RadioGroup, RadioGroupItem } from "@/components/ui/radio-group";
import { Slider } from "@/components/ui/slider";
import { generateTask } from "@/ai/flows/ai-task-generator";
import { useToast } from "@/hooks/use-toast";
import { createAdminTask, bulkCreateAdminTasks, deleteAdminTask, updateAdminTaskStatus } from "@/lib/actions";
                           ...
                           {updatingTaskId === task.id ? <Loader2 className="h-4 w-4 animate-spin ml-auto" /> : (
                             <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                    ...
                                    <DropdownMenuItem onClick={() => handleStatusToggle(task.id, task.status || 'Paused')}>
                                        {task.status === 'Active' ? <PauseCircle className="mr-2 h-4 w-4" /> : <PlayCircle className="mr-2 h-4 w-4" />}
