...
import { createAdminTask, bulkCreateAdminTasksChunk, deleteAdminTask, updateAdminTaskStatus, deleteAllAdminTasks } from "@/lib/admin-api";
...
function AutoGenerateDialog({
  ...
  const handleGenerate = async () => {
    ...
    for (let remaining = count; remaining > 0; remaining -= chunkSize) {
      chunkCounts.push(Math.min(chunkSize, remaining));
    }

    const requestedChunks = chunkCounts.length;
    setTotalChunks(requestedChunks);
    setCompletedChunks(0);
    setCreatedCount(0);
    setGenerationProgress(0);
    setGenerationStatus(`Started ${requestedChunks} backend request${requestedChunks === 1 ? "" : "s"}. Waiting for first completion...`);
    ...
    try {
      ...
      await Promise.all(
        chunkCounts.map(async (chunkCount) => {
          try {
            const result = await bulkCreateAdminTasksChunk({
              count: chunkCount,
              expertise,
            ...
            if (result.success) {
              const createdInChunk = Math.max(0, Number((result as { createdCount?: number }).createdCount || 0));
              createdSoFar += createdInChunk;
            } else {
              failedChunks += 1;
            }
          } catch {
            ...
            completedSoFar += 1;
            const percent = Math.min(100, Math.floor((createdSoFar / Math.max(1, count)) * 100));
            setCreatedCount(createdSoFar);
            setCompletedChunks(completedSoFar);
            setGenerationProgress(percent);
            setGenerationStatus(
              `Processed ${createdSoFar}/${count} contributions. ${completedSoFar}/${requestedChunks} backend request${requestedChunks === 1 ? "" : "s"} completed.`
            );
          }
        })
      );
      ...
      if (createdSoFar <= 0) {
        setGenerationStatus("Generation failed. No contributions were created.");
        toast({
          title: "Generation Failed",
          description: "No chunk succeeded. Please try again.",
          variant: "destructive",
        });
        return;
      }
      ...
      if (failedChunks > 0) {
        toast({
          ...
          description: `${createdSoFar} contribution${createdSoFar === 1 ? "" : "s"} created. ${failedChunks} backend request${failedChunks === 1 ? "" : "s"} failed.`,
        });
      } else {
        toast({
          title: "Success",
          description: `${createdSoFar} contribution${createdSoFar === 1 ? "" : "s"} created successfully.`,
        });
      }
          ...
          {(isGenerating || generationProgress > 0) && (
            <div className="rounded-md border p-3">
              <div className="flex items-center justify-between text-sm">
                <span>{generationStatus || "Preparing generation..."}</span>
                <span className="font-medium">{generationProgress}%</span>
              </div>
              <Progress value={generationProgress} className="mt-2 h-2" />
              <p className="mt-2 text-xs text-muted-foreground">
                Backend-confirmed progress: {createdCount}/{count} created, {completedChunks}/{totalChunks || 1} requests finished.
              </p>
            </div>
          )}
        </div>
